...
/// Definition of the JobWorker struct, responsible for managing and executing
/// jobs. When the server gets spawned it will spawn a `JobWorker` threads which
/// will be listening to requests from the main thread which in turn will be
/// prompted by the client messages.
///
/// The JobWorker listens to incoming requests in order to produce futures to
/// add to the `job_futures` as well as resolves the futures on the fly as
/// they're added to the `job_futures` set.
#[derive(Debug)]
pub struct JobWorker {
    /// OpenAI Client
    // TODO: Eventually generalise to other LLMs, in particular HuggingFace
    // compatible LLMs
    ...
    /// Collection of active job futures. Whenever a future gets added to this
    /// set it will be picked up the worker thread and resolved.
